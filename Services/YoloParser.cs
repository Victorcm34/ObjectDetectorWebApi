...
using ObjectDetectorWebApi.Interfaces;
using ObjectDetectorWebApi.Models;

namespace ObjectDetectorWebApi.Services;

public class YoloParser : IYoloParser
{
    ...
    public IEnumerable<YoloBoundingBox> ParseOutputs(float[] yoloModelOutputs, float threshold = 0.3F)
    {
        for (int row = 0; row < ROW_COUNT; row++)
        {
            for (int column = 0; column < COL_COUNT; column++)
            {
                for (int box = 0; box < BOXES_PER_CELL; box++)
                {
                    ...
                    var (topResultsIndex, topResultsScore) = GetTopResult(predictedClasses);
                    var topScore = topResultsScore * confidence;
                    if (confidence < threshold)
                        continue;

                    yield return new YoloBoundingBox{
                        Dimensions = new BoundingBoxDimensions(mappedBoundingBox),
                        Confidence = topScore,
                        Label = labels[topResultsIndex],
                        BoxColor = classColors[topResultsIndex]
                    };
                }
            }
        }
